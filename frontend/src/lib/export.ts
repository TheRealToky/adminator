...
/** One table's worth of data, used when bundling several tables into a single file. */
export interface ExportSheet {
  /** Stable machine key — used as the JSON object key. */
  key: string;
  /** Worksheet tab name (xlsx) / section title (csv). */
  sheetName: string;
  /** Human-readable label shown in CSV section headers. */
  label: string;
...
function sheetToAoa(sheet: ExportSheet): (string | number | boolean)[][] {
  return [
...
export function exportMultiSheet(
  ...
  if (format === 'json') {
    const payload = sheets.reduce<Record<string, Record<string, ExportCell>[]>>((acc, s) => {
      acc[s.key] = s.rows.map((row) =>
        s.columns.reduce<Record<string, ExportCell>>((obj, c) => {
