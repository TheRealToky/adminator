...
function MaterialsTab() {
    ...
    const columns = [
        ...
        { key: 'yield', header: t('processedMaterials.columns.yieldBatch'), align: 'right', render: (r) => (`${formatQuantity(r.yield_per_batch)} ${r.unit}`) },
        { key: 'cost', header: t('processedMaterials.columns.costUnit'), align: 'right', render: (r) => formatMoney(r.unit_cost) },
        { key: 'stock', header: t('processedMaterials.columns.onHand'), align: 'right', render: (r) => (_jsxs("span", { className: r.is_low ? 'text-red-600 font-semibold' : 'font-medium', children: [formatQuantity(r.stock_quantity), " ", r.unit] })) },
...
function RecipeModal({ open, material: materialProp, onClose, }) {
    ...
    return (_jsxs(Modal, { open: open, onClose: onClose, title: t('processedMaterials.recipe.title', { name: material?.name ?? '' }), size: "lg", children: [_jsx("p", { className: "text-sm text-slate-500 mb-4", children: _jsx(Trans, { i18nKey: "processedMaterials.recipe.lead", values: {
                        ...
                        unit: material?.unit ?? '',
                    }, components: { 1: _jsx("strong", {}) } }) }), _jsxs("div", { className: "grid grid-cols-12 gap-2 mb-2 px-3 text-xs uppercase tracking-wider text-slate-500", children: [_jsx("span", { className: "col-span-6", children: t('processedMaterials.recipe.ingredient') }), _jsx("span", { className: "col-span-3 text-right", children: t('processedMaterials.recipe.quantity') }), _jsx("span", { className: "col-span-2 text-right", children: t('processedMaterials.recipe.cost') }), _jsx("span", { className: "col-span-1" })] }), _jsxs("ul", { className: "divide-y divide-slate-100 border border-slate-200 rounded-md", children: [rows.length === 0 && (_jsx("li", { className: "px-3 py-6 text-center text-sm text-slate-400", children: t('processedMaterials.recipe.empty') })), rows.map((it) => (_jsxs("li", { className: "grid grid-cols-12 gap-2 px-3 py-2 items-center", children: [_jsxs("span", { className: "col-span-6 text-sm flex items-center gap-2", children: [it.ingredientName, it.isSub && (_jsx("span", { className: "badge-gray text-[10px]", title: t('processedMaterials.recipe.processedTip'), children: t('processedMaterials.recipe.processedBadge') }))] }), _jsxs("span", { className: "col-span-3 text-right text-sm", children: [formatQuantity(it.quantity), " ", it.ingredientUnit] }), _jsx("span", { className: "col-span-2 text-right text-sm text-slate-600", children: formatMoney(it.lineCost) }), _jsx("button", { className: "col-span-1 text-red-500 hover:text-red-700 justify-self-end", onClick: () => remove.mutate(it.id), children: _jsx(X, { size: 16 }) })] }, it.id)))] }), _jsx("div", { className: "mt-4 flex gap-1 text-xs", children: ([
...
function BatchesTab() {
    ...
    const columns = [
        ...
        { key: 'qty', header: t('processedMaterials.batches.columns.produced'), align: 'right', render: (r) => (`${formatQuantity(r.quantity_produced)} ${r.processed_material_unit}`) },
...
function MovementsTab() {
    ...
    const columns = [
        ...
        { key: 'delta', header: t('processedMaterials.movements.columns.delta'), align: 'right', render: (r) => (_jsxs("span", { className: Number(r.quantity_delta) >= 0 ? 'text-emerald-600 font-semibold' : 'text-red-600 font-semibold', children: [Number(r.quantity_delta) > 0 ? '+' : '', formatQuantity(r.quantity_delta), " ", r.item_unit] })) },
        { key: 'balance', header: t('processedMaterials.movements.columns.after'), align: 'right', render: (r) => (`${formatQuantity(r.balance_after)} ${r.item_unit}`) },
